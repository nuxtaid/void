  ...
  async message(peer, message) {
    ...
    if (action === 'xmastree') {
      ...
      if (!pythonProcess) {
        ...
        peer.send(
          JSON.stringify({
            status: 'xmastree-success',
            message: 'Success: Xmas tree started.',
          }),
        )
        return
      }
      if (pythonProcess) {
        ...
        peer.send(
          JSON.stringify({
            status: 'xmastree-success',
            message: 'Success: Xmas tree stopped.',
          }),
        )
        return
      }
...
function getUptime() {
  const startDate = new Date('Thu 12 Dec 2024 09:41:00 MST')
  const now = new Date()
